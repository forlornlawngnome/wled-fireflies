...
# Exponential brightness lookup table — matches human perception of light intensity.
# Generated by: round(pow(2.0, i/32.0) - 1) for i in 0..255
# Same formula used in the Arduino firefly reference implementation.
# Maps linear 0-255 brightness to perceptually linear 0-255 output.
_EXP_TABLE = [int(round(pow(2.0, i / 32.0) - 1)) for i in range(256)]
_EXP_TABLE = [min(255, max(0, v)) for v in _EXP_TABLE]

def perceptual(b_linear):
    """Convert linear brightness 0.0-1.0 to perceptually linear 0-255."""
    idx = min(255, max(0, int(b_linear * 255)))
...
def flash_intensity_saikia(t, dur, gamma):
    """Saikia & Bora (2020) nonlinear waveform.
    Asymmetric logistic rise then Gaussian decay — models luciferin-luciferase kinetics."""
    dt = t / dur
    if dt < 0:
        return 0.0
    elif dt < 0.15:
...
def flash_intensity_bezier(t, dur, gamma):
    """Bézier cubic waveform — matches the Arduino firefly archive approach.
    Models the flash as a smooth cubic curve with fast rise and slower decay,
    sampled from oscillograph recordings of real species (Lloyd 1966).
    gamma controls rise steepness: higher = sharper peak."""
    dt = t / dur
    if dt < 0 or dt > 1:
        return 0.0
    # Asymmetric cubic Bézier: fast rise to peak at ~30% of duration, slow decay
    # Control points tuned to match the archive's pyralis waveform shape
    rise_frac = max(0.1, min(0.4, 0.3 / (gamma / 4.0)))
    if dt < rise_frac:
        # Rising phase: cubic ease-in
        p = dt / rise_frac
        return 3 * p * p - 2 * p * p * p
    else:
        # Decay phase: cubic ease-out with gamma-controlled steepness
        p = (dt - rise_frac) / (1.0 - rise_frac)
        decay_exp = 1.0 + gamma * 0.15
        return max(0.0, (1.0 - p) ** decay_exp)


def flash_intensity(t, dur, gamma):
    """Dispatch to whichever waveform model is currently selected."""
    if state.get("waveform") == "bezier":
        return flash_intensity_bezier(t, dur, gamma)
    return flash_intensity_saikia(t, dur, gamma)
...
def api_status():
    return jsonify({
        ...
        "waveform": state.get("waveform", "saikia"),
...
def api_start():
    ...
    if "waveform" in data and data["waveform"] in ("saikia", "bezier"):
